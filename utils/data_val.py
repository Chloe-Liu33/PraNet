...
class PolypObjDataset(data.Dataset):
    ...
    def rgb_loader(self, path):
        with open(path, 'rb') as f:
            ...
            # 像素值扁平化，一维化
            #imgpixel = (imgCopy.flatten()).reshape((imgCopy.shape[0] * imgCopy.shape[1], 1))
            #kind = 2
            # kmeans = k_means_.KMeans(n_clusters=kind)
            #kmeans = KMeans(n_clusters=kind)
            #label = kmeans.fit(imgpixel)  # 对像素矩阵进行kmeans计算
            #img = np.array(label.labels_).reshape(imgCopy.shape)  # 将kmeans聚类后的结果取出来
...
class test_dataset:
    ...
    def load_data(self):
        ...
        ##increase k-means clustering，Liu Limei
